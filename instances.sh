...
numberofinstances=`cat -s $instanceslist | jq '[.data[]."display-name"],[.data[].id]' | grep "^  \"ocid1.instance.oc1" | wc -l`
...
do
    id[$counter]=`cat -s $instanceslist | jq '[.data['$counter'].id]' | grep "ocid1.instance.oc1" | sed 's/\"//g;s/\  //g'`
    name[$counter]=`cat -s $instanceslist | jq '[.data['$counter']."display-name"]' | grep \" | sed 's/\"//g;s/\  //g'`
    state[$counter]=`cat -s $instanceslist | jq '[.data['$counter']."lifecycle-state"]' | grep \" | sed 's/\"//g;s/\  //g'`
